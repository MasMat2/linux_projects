...
def playerend(playercolor):
    playerend =  {'rect':pygame.Rect(0,0, PILLW, PILLH)}
    playerend['rect'].bottom, playerend['rect'].left = player['rect'].bottom, player['rect'].right
    playerend['color'] = playercolor
    return playerend
...
print(x)
y = 0
player = {"rect":pygame.Rect(x , y, PILLW, PILLH), "color":WHITE}
playercolor = random.choice(COLORS)
sprites = [({"rect":pygame.Rect(144 , 336, PILLW, PILLH), "color":BLUE}),({"rect":pygame.Rect(168, 336, PILLW, PILLH), "color":BLUE}), ({"rect":pygame.Rect(144 , 360, PILLW, PILLH), "color":BLUE}), ({"rect":pygame.Rect(168 , 360, PILLW, PILLH), "color":BLUE})]
...
while True:
    ...
    for pill in sprites[:]:
        if not emptyspace(player['rect'], spaces):
            ...
            if bug not in sprites:
                nextsquare = playerend(playercolor)
                sprites.append(bug)
                sprites.append(nextsquare)
                newpos(bug)
                newpos(nextsquare)
    ...
    pygame.draw.rect(window_surface, playercolor, playerend(playercolor)["rect"])
    # playerend['rect'] = player['rect']
